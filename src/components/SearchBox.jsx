...
function ProductRow({ p, onGo }) {
  ...
  return (
    <button
      type="button"
      className="flex items-center gap-3.5 w-full px-4 py-3 bg-transparent border-0 cursor-pointer text-left hover:bg-[#edf1f5] transition-colors"
      onClick={() => onGo(`/proizvod/${p.slug}`)}
    >
      <div className="w-14 h-14 bg-white border border-gray-200 shrink-0 overflow-hidden flex items-center justify-center">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="m-0 mt-1 flex items-baseline gap-2">
          ...
          <span className={`text-[14px] font-bold ${p.old ? 'text-[#E02020]' : 'text-[#1e272e]'}`}>{fmtKM(p.price)}</span>
        </p>
      </div>
    </button>
  )
    ...
    <div className="flex flex-wrap items-center gap-2 px-4 py-3 border-b border-gray-100">
      {chips.map((c) => (
        <button
          key={c.slug}
          type="button"
          className="px-3.5 py-2 bg-gray-100 border-0 text-[13px] text-gray-800 cursor-pointer hover:bg-gray-200 transition-colors"
...
export default function SearchBox({ onNavigate }) {
    ...
    return (
      <div style={{ fontFamily: FONT }} className="flex-1">
        ...
        {open && (
          ...
          >
            <div className="bg-white flex flex-col min-h-0 overflow-hidden shadow-2xl">
            ...
            <button
              type="button"
              className="mx-auto mt-4 mb-1 w-14 h-14 bg-white border-0 flex items-center justify-center shadow-xl cursor-pointer shrink-0"
              onClick={close}
  ...
  return (
    <div ref={rootRef} className="relative flex-1 max-w-[460px]" style={{ fontFamily: FONT }}>
      ...
      {open && hasContent && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 shadow-[0_16px_32px_-12px_rgba(10,14,23,0.25)] z-[60] flex flex-col max-h-[70vh] overflow-hidden">
          <SuggestionList typed={typed} matches={matches} rows={rows} chips={chips} query={query} onGo={go} />
        </div>
      )}
    </div>

...
export default function CartDrawer() {
  ...
  return (
    <>
      {/* Scrim */}
      <div
        className={`fixed inset-0 bg-[#0A0E17]/40 z-[89] backdrop-blur-[2px] transition-opacity duration-200 ${drawerOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
        onClick={closeDrawer}
        aria-hidden="true"
      />
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
          <h2 className="text-[11px] font-bold tracking-[0.16em] uppercase text-[#0A0E17]">
            Korpa · {items.length} {items.length === 1 ? 'artikal' : 'artikala'}
          </h2>
          <button
            className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-[#0145F2] hover:bg-gray-100 transition-colors duration-150 cursor-pointer bg-transparent border-0"
            onClick={closeDrawer}
        ...
        <div className="flex-1 overflow-y-auto">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-4 text-center py-16 px-6">
              ...
              <div>
                <p className="text-[13px] font-semibold text-[#0A0E17] mb-1">Korpa je prazna</p>
            ...
            <div className="divide-y divide-gray-100">
              {items.map((item, idx) => (
                <div key={`${item.id}-${idx}`} className="flex gap-3 p-4 relative">
                  <div className="w-16 h-16 border border-gray-200 bg-[#F2F4F7] shrink-0 overflow-hidden">
                    <img src={item.img} alt={item.title} className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 min-w-0 pr-5">
                    <p className="text-[10px] font-bold tracking-[0.12em] uppercase text-gray-400 mb-0.5">{item.brand}</p>
                    <p className="text-[12px] font-semibold text-[#0A0E17] leading-snug line-clamp-2 mb-2">{item.title}</p>
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] text-gray-400">{item.qty} × {fmtKM(item.price)}</span>
                      <span className="text-[13px] font-bold text-[#0A0E17]">{fmtKM(item.price * item.qty)}</span>
                    </div>
                  </div>
                  <button
        ...
        <div className="border-t border-gray-200 p-5 flex flex-col gap-3">
          ...
          {nudge > 0 && items.length > 0 && (
            <div className="flex items-center justify-between py-2.5 px-3.5 border border-gray-200 bg-[#F2F4F7]">
              <span className="text-[11px] text-gray-500">
                Još <strong className="text-[#0A0E17] font-bold">{fmtKM(nudge)}</strong> do besplatne dostave
              </span>
              <div className="w-24 h-1 bg-gray-200 rounded-full overflow-hidden">
                <div
                  className="h-full bg-[#0145F2] rounded-full transition-all duration-300"
          ...
          <div className="flex items-center justify-between py-1">
            ...
            <span className="text-xl font-bold text-[#0A0E17]" style={{ fontFamily: "'Exo 2', system-ui, sans-serif" }}>
              {fmtKM(totalPrice)}
            </span>
          </div>
          ...
          <button
            className="w-full py-3 border border-gray-300 text-[#0A0E17] text-[11px] font-bold tracking-[0.1em] uppercase hover:border-gray-400 transition-colors duration-150 cursor-pointer bg-transparent"
            onClick={closeDrawer}
          >
            Nastavi kupovinu
          </button>

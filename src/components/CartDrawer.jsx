...
export default function CartDrawer() {
  ...
  return (
    <>
      {/* Scrim */}
      <div
        className={`fixed inset-0 bg-[#1e272e]/40 z-[89] backdrop-blur-[2px] transition-opacity duration-200 ${drawerOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
        onClick={closeDrawer}
        aria-hidden="true"
      />
            ...
            <div className="divide-y divide-gray-100">
              {items.map((item, idx) => (
                <div key={`${item.id}-${idx}`} className="flex gap-3 p-4 relative">
                  <div className="w-16 h-16 border border-gray-200 bg-[#edf1f5] shrink-0 overflow-hidden">
                  ...
                  <div className="flex-1 min-w-0 pr-5">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[13px] font-bold text-[#1e272e]">{fmtKM(item.price * item.qty)}</span>
                    </div>
                  </div>
                  <button
